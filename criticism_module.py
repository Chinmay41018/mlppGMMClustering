...
def criticise(path_to_d):
    ...
    labels = pd.read_csv(
        ...
        header=0,
        names=['text', 'label', 'embedding']).labels
    encoded_label = [en[y] for y in labels]
    model = LogisticRegression()
    cv_score = cross_validate(model, dat.tolist(), encoded_label, cv=3)
    print("Classification performance : \n", np.mean(cv_score))
    return np.array([x.data.numpy() for x in dat]), labels
